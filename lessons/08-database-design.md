...
1. 外部キー制約は参照整合性を保証し、onDelete で削除時の動作を指定できます。

2. インデックスは検索を高速化しますが、WHERE句で使うカラムに追加し、書き込み性能とのトレードオフを考慮してください。

3. NULL許可は比較・集計が複雑になるため、可能なら NOT NULL + デフォルト値を使いましょう。

4. 複合ユニーク制約は重複を防ぎ、アプリケーションレベルより確実です。

5. マイグレーションは小さな単位で作成し、down() を実装し、破壊的変更は段階的に行いましょう。
