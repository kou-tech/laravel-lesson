...
## Step 7 Eloquentコレクション固有のメソッド

### find - IDで検索

```php
$courses = Course::all();
$course = $courses->find(1);  // ID=1の講座
```
...
```php
$courses = Course::all();
$courses->load('instructor');  // 後からリレーションをロード
```

### modelKeys - モデルのIDを取得

```php
$ids = $courses->modelKeys();  // [1, 2, 3, ...]
```

### fresh - 最新のデータを再取得

```php
$courses = $courses->fresh();  // DBから再取得
```
...
このレッスンでは、コレクションとコレクションメソッドを学びました。map、filter、pluckなどの基本メソッドを使いこなすことで、データ操作が直感的に書けます。メソッドチェーンで複雑な処理も簡潔に表現できますが、パフォーマンスを意識してDBクエリとの使い分けが重要です。
...
### 問題2
講座一覧をステータスごとにグループ化し、各ステータスの件数を取得するコードを書いてください。
