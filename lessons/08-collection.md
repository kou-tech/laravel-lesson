...
> サンプル中の `Course` は Lesson 9 で作るモデルの先取りです。また `$course->status === 'active'` のような比較を例に使っていますが、Lesson 9 以降の `Course` は `status` を Enum にキャストするため、実際のコードでは `$course->status === CourseStatus::Active` と書きます。ここではコレクションメソッドの挙動に集中してください。
