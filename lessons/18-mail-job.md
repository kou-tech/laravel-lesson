...
> なぜトランザクションの外でディスパッチするのか: トランザクション内でジョブを積むと、まだコミットされていないレコードをワーカーが処理しにいく可能性があります（ワーカーは別プロセスなので、コミット前のデータは見えません）。結果として「該当レコードが見つからない」でジョブが失敗します。次に説明する `afterCommit()` を使う方法でも同じ問題を回避できます。
