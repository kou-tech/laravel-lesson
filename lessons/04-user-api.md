...
1. DBのカラム構成がそのまま外部に露出している
   `role` や `two_factor_confirmed_at` は、プロフィールを表示したいだけのクライアントには不要な情報です。テーブル定義がそのままAPIの仕様になってしまうと、カラムを追加・リネームしただけでAPIの返す内容が勝手に変わります。DBの都合とAPIの都合を切り離すのが、API Resourceの役割です。

2. `$hidden` は「全ての用途で隠す」しかできない
   `$hidden` はモデル単位の設定なので、「管理者向けAPIでは `role` を返したいが、一般向けでは隠したい」といった用途ごとの出し分けができません。API Resourceならエンドポイントごとに別のResourceを用意できます。
