...
Step 2 のコードは、モデルをそのまま返しています。実際に `GET /api/users/1` を叩くと、以下が返ってきます。
...
`password` が入っていないのは、`User` モデルの `$hidden` に指定されているからです（Lesson 13 で詳しく扱います）。ただし `$hidden` で守れるのはここまでで、以下の問題は残ります。

問題点

1. DBのカラム構成がそのまま外部に露出している
   `role` や `two_factor_confirmed_at` は、プロフィールを表示したいだけのクライアントには不要な情報です。テーブル定義がそのままAPIの仕様になってしまうと、カラムを追加・リネームしただけでAPIの返す内容が勝手に変わります。**DBの都合とAPIの都合を切り離す**のがAPI Resourceの役割です。

2. `$hidden` は「全ての用途で隠す」しかできない
   `$hidden` はモデル単位の設定なので、「管理者向けAPIでは `role` を返したいが、一般向けでは隠したい」といった**用途ごとの出し分け**ができません。API Resourceならエンドポイントごとに別のResourceを用意できます。

3. レスポンスの形式が統一しにくく、コードが重複する
   ユーザー詳細API・ユーザー一覧API・ユーザー更新APIの全てで同じフィールドを返したい場合、各メソッドに同じ整形ロジックを書く必要があります。後から「`created_at` のフォーマットを変えたい」「フィールドを追加したい」となった場合、全ての箇所を修正しなければならず、修正漏れのリスクが高まります。
...
- 指定したフィールドのみが含まれている（`role` や `two_factor_confirmed_at` は消えた）
- `created_at` が `format('Y-m-d H:i:s')` の指定どおりの形式になっている
